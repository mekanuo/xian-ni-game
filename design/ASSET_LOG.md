...
2026-09-11，内置 image_gen 原创生成并定向修图，未使用 CLI/API 后备。四张最终母版已目视核对：public/assets/kiln-environment.png（1536×1024，窑墙/压顶/布棚/陶架）、kiln-props.png（1536×1024，完整屏/烧后残片）、kiln-duqin.png（1254×1254，独立陶工杜芹）、kiln-ground.png（1536×1024，平地材质）。前三为 RGB 洋红底，由既有 keyAtlas 运行时去底；不是原生透明 PNG。地面没有烘焙地图障碍。

生成目录尾段 01a08e74-9183-7932-87b0-10da57037ee8，对应原始输出依次为 exec-cdd37f08-6061-4d4a-ba37-15ee0c67fc90.png、exec-aeb8ddf5-c019-4f38-8927-a31523ec9a1c.png、exec-d7c8c1af-3b91-4409-8da2-93e496bfd504.png、exec-a90f80fc-332e-4d2e-85b9-372becc8031a.png。精确裁帧和真实状态绘制见 src/game/kiln-art.ts；沿用已有 detail-props/rest 坐垫。

提示方向：低饱和陶土/灰蓝、上左漫射光、约3.5头身成年女陶工（围裙、束发、刮刀），完整横屏与同物烧毁残片；布棚与窑墙只作独立素材，不生成带假路障的整幅地图。杜芹不是旧人物换名。人物为独立静态原画加程序状态，未制作完整逐帧动画。首轮正式桌面截图没有粉底或重图，但地面碎纹和墙体立面感仍在收敛；不能把资产生成或初次截图视为最终美术验收。


## 0.4 雾岭旧渠

2026-09-11。新增三张内置 image_gen 原创生成图，提示词、修图过程与精确裁帧见 [ART_CANAL_040](ART_CANAL_040.md)：

| 文件 | 尺寸 | 用途 |
|---|---|---|
| public/art/canal-ground.png | 1536×1024 | 平地石路、草苔与边缘材质，不烘焙可变化水面或虚假障碍。 |
| public/art/canal-architecture.png | 1536×1024 | 取水瓦檐、截水架、分水槽、台阶；运行时色键去背景。 |
| public/art/canal-props.png | 1254×1254 | 两块活动板、筛框清理前后、石槽、水轮、水尺、独立邵禾人物和水桶。 |

渠床、石盖板从上述建筑原画石面取样后运行时烘焙；水位、筛框状态、水轮和桶的位置只读取真实规则。没有第三方截图或小说原文进入运行资产。实机矩形湿界、器物脚点和三视口证据位于 qa/evidence/canal-view.json；最终发行状态以 qa/verification.json 为准。
