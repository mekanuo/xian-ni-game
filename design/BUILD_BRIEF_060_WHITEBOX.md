...
最小状态沿用GameState、Entity、Projectile和现有act/tick；白盒新增eastReached/returned仅根据身体抵达固定端点，不写入游戏flags。普通/零灵力按钮分别创建固定6/0灵力初态，玩家不在运行中任意补资源。新开重置整个切片，无存档导入、剧情调试开关或完成指令。规则源继承a67c479加本分支燃烧物状态保持与solid挡屏完整足迹补丁，白盒内容修订kiln-whitebox-3（北侧围墙上移40，东侧转身空间加宽60），无随机生成；无白盒save schema。
