## 2026-09-11 · 0.4 规则与场景整合候选

- 隔离分支 feat/canal-040 已集成第五图雾岭旧渠，两法清渠→验水→人物确认→实际归驿落笔；旧章结局事实保留。
- 内容版本3升级0.2/0.3与单层恢复点；唯一压扣和所属场景药囊，真实操作时暂停/中断一致。
- 三张新原画、邵禾独立帧、共用碰撞水线、原画机关、水轮和桌案小图已接入。尚未完成实机画面审阅。
- npm test：223/223，npm run build：exit0。新增整合测试使用真实模型act/tick从旧档走完两法；浏览器证据尚未取得，不声明手机留势窗已通过。
- 审阅修复：外缘山兽追咬/同伴受阻、同伴抄近路入干渠、筛框自动站位落在岸外、清理中释放支撑后瞬时存档无效。读档意外恢复/重弹旧结局另补10项UI方法测试，待回补0.3.1。
- 下一步：排程scripts/canal-check.mjs，目视桌面/手机/矮窗与真实存读档，修正后全量发行测试。公网保持稳定0.3.0，主目录0.3.1正在完成检查。
