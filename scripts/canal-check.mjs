...
async function importFixture(bytes){
 ...
 const s=await state();assert.equal(s.contentVersion,4);assert.equal(s.scene,'home');assert.equal(s.canal.stage,'unaccepted');assert.equal(s.life.clamp,'unowned');assert.equal(s.life.sachets,0);assert.equal(JSON.parse(s.checkpoint).contentVersion,4);route.observations.import=brief(s);
}
