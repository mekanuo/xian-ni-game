...
async function importFixture(bytes){
 ...
 const s=await state();assert.equal(s.contentVersion,3);assert.equal(s.scene,'home');assert.equal(s.canal.stage,'unaccepted');assert.equal(s.life.clamp,'unowned');assert.equal(s.life.sachets,0);assert.equal(JSON.parse(s.checkpoint).contentVersion,3);route.observations.import=brief(s);
}
