...
try{
 ...
 for(const [id,path]of Object.entries(paths)){
  const bytes=await readFile(path),s=JSON.parse(bytes.toString('utf8'));assert.equal(s.contentVersion,7);assert.deepEqual(Object.keys(s.worlds).sort(),['canal','creek','crossing','home','kiln','market','spar','workshop']);assert.equal(s.canal.stage,'complete');assert.equal(s.ended,true);
