...
try{
 ...
 for(const device of devices){
  ...
  await wait(time=>{const s=window.__XIAN_NI__.inspect();return s.contentVersion===7&&s.scene==='home'&&s.time===time&&s.paused;},original.time);await page.waitForTimeout(160);
  assert.equal(await page.getByRole('button',{name:'在驿中再坐一会儿',exact:true}).isVisible(),false);let s=await state();assert.deepEqual(Object.keys(s.worlds).sort(),['canal','creek','crossing','home','kiln','market','spar','workshop']);assert.deepEqual(s.journey,original.journey);assert.equal(s.player.mana,0);assert.deepEqual(s.kiln,{visited:false,entry:null,crossed:{west:false,east:false},loan:'none',shelterOpened:false});item.migration={sourceVersion:4,resultVersion:7,time:s.time,journeyPreserved:true,kiln:s.kiln};
