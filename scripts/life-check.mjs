...
try{
 ...
 const loaded=await state();assert.equal(loaded.contentVersion,3);assert.equal(loaded.life.repair.stage,'unaccepted');assert.equal(loaded.life.harvest.stage,'unaccepted');assert.equal(loaded.life.clamp,'unowned');assert.equal(loaded.life.sachets,0);
 assert.equal(JSON.parse(loaded.checkpoint).contentVersion,3);evidence.observations.migration={state:brief(loaded),checkpointContentVersion:JSON.parse(loaded.checkpoint).contentVersion};log('legacy-import',brief(loaded));
