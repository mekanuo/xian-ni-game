...
try{
 ...
 const bytes=await readFile(fixturePath),old=JSON.parse(bytes);if(resumed)assert.ok([6,7].includes(old.contentVersion),'Continuation must be a real v6/v7 market export');else assert.equal(old.contentVersion,5);if(resumed){assert.equal(old.scene,'market');assert.equal(old.market.visit.entry,'crossing');}assert.equal(old.kiln.crossed.east,true);assert.equal(old.kiln.crossed.west,true);
 evidence.fixture={path:fixturePath,sha256:hash(bytes),contentVersion:old.contentVersion,source:resumed?'Actual earlier market UI entry export; this run resumes there through settings, without synthetic edits.':'Actual 0.6 two-ended kiln settings export, unchanged; every new scene transition and outcome below uses normal UI input.'};
