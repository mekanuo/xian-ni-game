...
try{
 ...
 for(const device of devices){
  ...
  await wait(time=>{const s=window.__XIAN_NI__.inspect();return s.contentVersion===6&&s.scene==='market'&&s.paused&&s.time===time;},original.time);await page.waitForTimeout(160);
  const restored=await state();for(const key of ['player','market','kiln','journey','worlds','pending','dialogue','time'])assert.deepEqual(restored[key],original[key],`Import must preserve actual fixture ${key}`);assert.equal(await page.locator('.modal-paper').isVisible(),false);assert.equal(await page.getByRole('button',{name:'在驿中再坐一会儿',exact:true}).isVisible(),false);
