...
try{
 ...
 await interact('xu');await choose('invite');await exit('to_creek','creek');await interact('rope');await choose('fix');await interact('bag');assert.equal((await state()).flags.tools,true);
 ...
 await interact('tao_work');await choose('hold');await walk(1390,610);await page.locator('[data-ui="spell:pull"]').click();await click(1530,580);await page.waitForFunction(()=>window.__XIAN_NI__.inspect().player.pullId==='trial');await click(1440,580);await page.waitForTimeout(650);await page.locator('[data-ui="hold"]').click();assert.equal((await state()).ringStyle,'hold');await page.screenshot({path:'qa/evidence/hold-local.png'});await page.locator('[data-ui="spell:ward"]').click();const wardAim=await point(1510,610);await page.mouse.move(wardAim.x,wardAim.y);await page.screenshot({path:'qa/evidence/hold-ward-aim.png'});await page.locator('[data-ui="release"]').click();log('mouse-hold-training','learned through visible button');
 await interact('tao_work');await choose('long');await walk(1210,600);await pull('trial',1310,600);
