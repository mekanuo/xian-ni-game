...
export type JourneyRoute = 'north' | 'south' | 'mixed';
export type JourneyMode = 'solo' | 'together';
export interface JourneyState {
 stage: 'unaccepted' | 'active' | 'ready' | 'complete';
 agreed: JourneyMode | null;
 run: null | {mode: JourneyMode; plan: 'north' | 'south'; next: number | null; playerGate: boolean; companionGate: boolean; viaSouth: boolean; waiting: boolean};
...
export type MarketDirection = 'westToEast' | 'eastToWest';
export type MarketPassages = Record<MarketRoute, Record<MarketDirection, boolean>>;
export interface MarketState {
 exchanged: boolean;
 visit: null | {entry: 'crossing' | 'canal'; transit: null | {route: MarketRoute; from: 'west' | 'east'}; passed: MarketPassages};
 through: MarketPassages;
 reported: MarketPassages;
}
