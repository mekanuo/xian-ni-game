...
export function validateLife(l:LifeState):void {
 ...
 if(!['unpicked','bag','upper','lower'].includes(l.harvest.sun)||!['unpicked','bag','upper','lower'].includes(l.harvest.shade)||l.harvest.sun===l.harvest.shade&&l.harvest.sun!=='unpicked')throw Error('叶片层级无效');
 if(l.harvest.picking&&(!['life_sun_leaf','life_shade_leaf'].includes(l.harvest.picking.id)||!finite(l.harvest.picking.elapsed)||l.harvest.picking.elapsed<0||l.harvest.picking.elapsed>2))throw Error('采叶计时无效');
