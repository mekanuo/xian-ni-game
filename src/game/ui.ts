...
export class GameUI {
  ...
  update() {
    ...
    const hudKey=JSON.stringify([s.scene,s.player.hp,s.player.mana,s.selected,s.paused,s.ringStyle,Boolean(s.player.pullId),near?.id,objective(s),Boolean(s.dialogue),s.defeated,this.screen,s.flags.ringOwned,s.flags.trialStyle,this.audioState,aiming,panning,s.pending?.type]);
