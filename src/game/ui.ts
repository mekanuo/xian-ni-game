...
export class GameUI {
  ...
  private showCreate() {
    this.screen = 'create';
    this.shell('你从哪里来', `<p class="muted">你在回石驿住了半年，今日要去取回自己的引环。</p><form id="new-game"><label class="name-label">姓名 <input name="name" maxlength="12" value="行舟" autocomplete="off" aria-label="修士姓名"></label><div class="creation-layout"><div><h3>已有的手艺</h3><div class="option-row"><label class="choice-tile"><input type="radio" name="origin" value="herbalist" checked><strong>药铺帮工</strong><span>认草药，会包扎。许照常与你同路。</span></label><label class="choice-tile"><input type="radio" name="origin" value="tinker"><strong>修器学徒</strong><span>懂绳扣和受力。陶七给你留着工位。</span></label></div><h3>眼下的心愿</h3><div class="option-row"><label class="choice-tile"><input type="radio" name="wish" value="travel" checked><strong>有本领走出去</strong><span>想看看山外的路。</span></label><label class="choice-tile"><input type="radio" name="wish" value="stay"><strong>有自己的修行处</strong><span>想留下一盏自己的灯。</span></label></div><div class="appearance-choice"><span>形象</span><label><input type="radio" name="appearance" value="0" checked> 束发青衫</label><label><input type="radio" name="appearance" value="1"> 挽髻短襟</label></div></div><div class="character-preview"><div class="portrait portrait-player"></div><small>凝气三层 · 初走远路</small></div></div><div class="modal-actions">${this.button('回到山外','title','secondary')}<button type="submit" class="primary">去回石驿</button></div></form>`, 'creation-paper');
  ...
  update() {
    ...
    if(s.ended&&!this.endingShown) { this.endingShown=true; this.notify('这一程已走完。你仍可以在驿中停留，也可以再去山道试术。'); this.save(); }
  }
}
