...
export class GameUI {
  ...
  private drawModal() {
    ...
    if (this.screen==='journal') {
      ...
      const marketRoutes=(['public','private'] as const).flatMap(route=>(['westToEast','eastToWest'] as const).filter(direction=>s.market.through[route][direction]).map(direction=>`${route==='public'?'公共巷':'私巷'}：${direction==='westToEast'?'石渡至旧渠':'旧渠至石渡'}`));
      const marketCard=s.flags.visited_market?`<article><span class="item-glyph">巷</span><h3>小集走法</h3><p>${marketRoutes.length?marketRoutes.map(esc).join('<br>'):'已经到过小集，还没有从另一端走出。'}</p><p>${s.market.exchanged?'旧窑路况已当面告诉沈砚；门要等她亲自到闩边才能打开。':'公共巷可以自行通行；私巷可近身向沈砚问路。'}</p></article>`:'';
      const lifeCards=repairCard+harvestCard+rewardCard+canalCard+journeyCard+marketCard;
      ...
      this.shell('行路须知',`<div class="help-grid"><p><kbd>点击地面 / W A S D</kbd><span>行走；点远处的人或物会先走近</span></p><p><kbd>下方术法 / 1 2 3</kbd><span>选术法，再点击目标施术</span></p><p><kbd>空格</kbd><span>暂停观察，再按继续</span></p><p><kbd>E</kbd><span>与近处的人、物互动</span></p><p><kbd>察看周围 / Q</kbd><span>展开眼前处境和可尝试的办法</span></p><p><kbd>R</kbd><span>学会留势后，让牵住的物件停留</span></p><p><kbd>Esc / 右键</kbd><span>取消瞄准或放下牵物</span></p><p><kbd>看路</kbd><span>开启后拖动画面查看远处；再次点击恢复行走</span></p><p><kbd>回到身边 / C</kbd><span>镜头重新跟随你</span></p><p><kbd>J / I / M</kbd><span>记事 / 行囊 / 山道图</span></p></div><p class="muted">引力术牵住轻物后，点击可达地面放下。暂停时可预先指定一个动作。灵力不足时，退到歇脚处静息即可恢复。</p><div class="modal-actions">${this.button(!this.started?'回到山外':'继续前行',!this.started?'title':'resume','primary')}</div>`);
  ...
  private click(e: MouseEvent) {
    ...
    else if (a==='pan') {this.hooks.pan();this.update();}
    ...
    else if (['release','hold','rest','heal','retry','retreat','use-sachet','spar-begin','spar-stop'].includes(a)) { if(a==='use-sachet')this.close(); this.hooks.act({type:a} as GameAction); this.update(); if (a==='heal') this.drawModal(); }
  ...
  update() {
    ...
    const aiming=this.hooks.aiming(),panning=this.hooks.panning();
    const hudKey=JSON.stringify([s.scene,s.player.hp,s.player.mana,s.selected,s.paused,s.ringStyle,Boolean(s.player.pullId),near?.id,objective(s),Boolean(s.dialogue),s.defeated,this.screen,s.flags.ringOwned,s.flags.trialStyle,this.audioState,aiming,panning,s.pending?.type,s.spar.run?.phase]);
    if(hudKey!==this.lastHud) {
      this.lastHud=hudKey;
      this.hud.innerHTML=`<div class="top-left paper hud-paper"><span class="name-mark">${esc(s.profile.name.slice(0,1))}</span><div><strong>${esc(s.profile.name)}</strong><span class="realm">凝气 · 三层</span><div class="resource hp" aria-label="体力 ${s.player.hp}/4"><span>体力</span>${[0,1,2,3].map(i=>`<i class="${i<s.player.hp?'full':''}"></i>`).join('')}</div><div class="resource mana" aria-label="灵力 ${s.player.mana}/6"><span>灵力</span>${[0,1,2,3,4,5].map(i=>`<i class="${i<s.player.mana?'full':''}"></i>`).join('')}</div></div></div><div class="location"><span>${SCENES[s.scene].subtitle}</span><h2>${SCENES[s.scene].title}</h2><p>${esc(objective(s))}</p></div><nav class="top-right">${this.button('山道','map')}${this.button('记事','journal')}${this.button('行囊','bag')}${this.button('设置','settings')}<button data-ui="pan" class="pan-toggle" aria-pressed="${panning}" title="切换拖动镜头；再次点击恢复行走点选" ${s.defeated?'disabled':''}>${panning?'看路中':'看路'}</button>${this.audioButton()}</nav><div class="pause-state ${s.paused&&!s.dialogue&&!this.screen?'shown':''}">${this.button('Ⅱ 已暂停 · 点击继续','pause','paper')}</div><div class="bottom-left"><span>WASD / 点击 · 行走</span><span>空格 · 暂停　C · 回到身边</span>${this.button('回到身边','center','touch-center paper')}</div><div class="action-dock paper"><div class="spells">${(['pull','flame','ward'] as Spell[]).map((spell,i)=>`<button data-ui="spell:${spell}" class="spell ${s.selected===spell?'selected':''}" aria-pressed="${s.selected===spell}" ${s.scene==='spar'&&spell!=='ward'?'disabled':''} title="${['牵引轻物；再点地面放下','点燃干物、打断敌人','向瞄准方向护持一次'][i]}"><kbd>${i+1}</kbd><span class="spell-symbol">${['引','焰','障'][i]}</span><span>${['引力术','火焰球','护符·障'][i]}</span></button>`).join('')}</div><div class="dock-divider"></div>${s.scene==='spar'&&s.spar.run?.phase==='positioning'?this.button('站稳开始','spar-begin','small-button'):''}${s.scene==='spar'&&s.spar.run?this.button('收手','spar-stop','small-button'):''}${this.button(s.paused?'▶ 继续':'Ⅱ 暂停','pause','pause-button')}${s.pending?.type==='use-sachet'?this.button('取消放药','cancel','small-button'):''}${aiming&&(!s.player.pullId||s.selected!=='pull')?this.button('收术','cancel','small-button'):''}${s.player.pullId?this.button('放下','release','small-button'):''}${s.player.pullId&&(s.ringStyle==='hold'||s.flags.trialStyle==='hold')?this.button('R 留势','hold','small-button'):''}</div><div class="nearby ${near&&!s.dialogue&&!this.screen?'shown':''}">${near?this.button(`<kbd>E</kbd> ${esc(near.name)}`,'interact','paper'):''}</div><button class="help-toggle" data-ui="help" aria-label="操作帮助">?</button>`;
    ...
    if(dk!==this.lastDialogue) {
      ...
      if(s.defeated)this.dialogues.innerHTML=`<div class="defeat-wrap"><section class="paper defeat"><span class="eyebrow">暂且退一步</span><h2>气息未稳</h2><p>${esc(s.events.at(-1)?.text||'受击后失去了行动余力。')}</p><div class="modal-actions">${this.button(s.scene==='spar'?'恢复上次完整落点':'重试当前处境','retry','primary')}${s.scene==='spar'?'':this.button('撤回歇脚处','retreat','secondary')}</div></section></div>`;
