...
export interface SparRound {s:GameState;peer:Entity;phase:'idle'|'positioning'|'active'|'settling'|'result';outcome:SparOutcome|null;shots:number;startHp:number;startSeq:number;blocked:boolean;stopReason:'stopped'|'outside'|null;}
export type SparAction=GameAction|{type:'agree'|'begin'|'stop'};
export function createSpar(mana=6):SparRound {
 ...
 return {s,peer,phase:'idle',outcome:null,shots:0,startHp:4,startSeq:0,blocked:false,stopReason:null};
}
...
export function sparAct(r:SparRound,a:SparAction):ActionResult {
 ...
 if(a.type==='agree'){
  ...
  r.startHp=s.player.hp;r.startSeq=Number(s.flags.projectileSeq??0);r.phase='positioning';r.outcome=null;r.stopReason=null;r.shots=0;r.blocked=false;r.peer.state='peaceful';r.peer.data!.attack=0;return {ok:true};
 }
 if(a.type==='begin'){
  if(r.phase!=='positioning'||!inside(r)||distance(r.peer,SPAR_PEER_READY)>3||!cleanStart(r))return fail('双方先走到位置，你站稳在线内。');
...
export function sparTick(r:SparRound,dt:number,input:Vec):void {
 ...
 while(remaining>1e-8){
  ...
  if(r.phase==='positioning')simulationPorts.moveNpc(s,r.peer,SPAR_PEER_READY,90,step);
