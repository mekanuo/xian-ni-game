...
describe('kiln v5 save migration',()=>{
 for(const [path,raw] of [...Object.entries(oldFixtures),['actual v4 complete',journeyComplete],['actual v4 paused guide',journeyPaused]])it(`upgrades ${path} and its checkpoint without changing prior facts`,()=>{
  ...
  for(const [next,old] of [[s,prior],[JSON.parse(s.checkpoint!),JSON.parse(prior.checkpoint)]]){
   expect(next.contentVersion).toBe(7);
   expect(Object.keys(next.worlds).sort()).toEqual(['canal','creek','crossing','home','kiln','market','spar','workshop']);
 ...
 for(const value of [0,1,8,'5',null])it(`rejects unsupported version ${String(value)}`,()=>{const s=JSON.parse(journeyComplete);s.contentVersion=value;expect(()=>restore(JSON.stringify(s))).toThrow();});
...
describe('kiln pause and spell save boundaries',()=>{
 it('round-trips a newly created current eight-map world and its original retry point',()=>{const s=createGame({name:'存档校验',origin:'tinker',wish:'travel',appearance:0});expect(round(s)).toEqual(s);expect(JSON.parse(s.checkpoint!).contentVersion).toBe(7);});
