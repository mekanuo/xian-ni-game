...
describe('actual model.restore save entry',()=>{
 ...
 for(const route of ['main','ridge'])it(`migrates real ${route} return export and its one checkpoint`,()=>{const old=fixture(route),s=restore(JSON.stringify(old));expect(s.contentVersion).toBe(6);expect(s.life.repair.stage).toBe('unaccepted');expect(s.player).toEqual(old.player);expect(s.flags).toEqual(old.flags);expect(s.ended).toBe(old.ended);for(const id of scenes){expect(s.worlds[id].filter(e=>!lifeEntityIds(id).includes(e.id)&&!initialJourneyEntities(id).some(n=>n.id===e.id)&&!initialKilnEntries(id).some(n=>n.id===e.id)&&!initialMarketEntries(id).some(n=>n.id===e.id))).toEqual(old.worlds[id]);expect(s.worlds[id]).toHaveLength(old.worlds[id].length+lifeEntityIds(id).length+initialJourneyEntities(id).length+initialKilnEntries(id).length+initialMarketEntries(id).length);}const nested=JSON.parse(s.checkpoint!);expect(nested.contentVersion).toBe(6);expect(nested.life).toEqual(s.life);expect(nested.checkpoint).toBeNull();expect(snapshot(round(s))).toBe(snapshot(s));});
 ...
 for(const version of [0,1,7,'2',null])it(`rejects unknown version ${String(version)}`,()=>{const s=current();Object.assign(s,{contentVersion:version});expect(()=>round(s)).toThrow();});
