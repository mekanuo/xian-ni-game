...
const scenes=Object.keys(SCENES) as SceneId[];
...
describe('actual model.restore save entry',()=>{
 ...
 for(const route of ['main','ridge'])it(`migrates real ${route} return export and its one checkpoint`,()=>{const old=fixture(route),s=restore(JSON.stringify(old));expect(s.contentVersion).toBe(2);expect(s.life.repair.stage).toBe('unaccepted');expect(s.player).toEqual(old.player);expect(s.flags).toEqual(old.flags);expect(s.ended).toBe(old.ended);for(const id of scenes){expect(s.worlds[id].filter(e=>!lifeEntityIds(id).includes(e.id))).toEqual(old.worlds[id]);expect(s.worlds[id]).toHaveLength(old.worlds[id].length+lifeEntityIds(id).length);}const nested=JSON.parse(s.checkpoint!);expect(nested.contentVersion).toBe(2);expect(nested.life).toEqual(s.life);expect(nested.checkpoint).toBeNull();expect(snapshot(round(s))).toBe(snapshot(s));});
 ...
 for(const version of [0,1,3,'2',null])it(`rejects unknown version ${String(version)}`,()=>{const s=current();Object.assign(s,{contentVersion:version});expect(()=>round(s)).toThrow();});
 ...
 for(const bad of [-1,0,9])it(`rejects impossible scent time ${bad}`,()=>{const s=harvest();Object.assign(s.life.harvest,{stage:'complete',sun:'upper',shade:'lower'});s.life.scent={remaining:bad};e(s,'life_scent').state='idle';expect(()=>round(s)).toThrow();});
 it('rejects scent with no visible source',()=>{const s=harvest();Object.assign(s.life.harvest,{stage:'complete',sun:'upper',shade:'lower'});s.life.scent={remaining:4};expect(()=>round(s)).toThrow();});
});
